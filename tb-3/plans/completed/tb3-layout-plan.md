...
| Item | Status | Notes |
|------|--------|-------|
| **BENDER RANGE scaling** | ✅ Resolved | Found (and fixed — see `receivingPatch` note below) an active corruption bug: `REGISTRY` used `max=23` to normalise the received raw byte while `ENC_SEND_MAP` used `max=17` to re-derive and resend it, so every "SYNC FROM TB-3" silently rewrote the hardware value (confirmed live: 17 → 13 → 10 across successive syncs). Once the echo was fixed, the user probed the real range by temporarily raising `max` to 127 and sliding to find where the TB-3 stopped responding — confirmed **raw 0–23**, where raw `N` = **±(N+1) semitones** (0 = ±1 ... 23 = ±24). Set `max=23` in both `ENC_SEND_MAP` and the `REGISTRY` entry, and added a `semitoneRange` flag that both `enc_moved` (root.lua) and `applyValue` (patch_manager.lua) check to render the label as "±N st" instead of the raw byte. |
| **SysEx echo on patch receive** | ✅ Fixed | Root cause of the BENDER RANGE corruption above, and a latent risk for any future scaling mismatch: `applyValue()` sets fader/switch values directly, which synchronously re-triggers `enc_moved`/`sw_toggled` → a real SysEx write back to the TB-3 — for every field in every block, on every sync *and* every restore. Added a `receivingPatch` guard flag (set/cleared synchronously around `parseBlock()` calls in `handleTB3SysEx` and `onReceiveOSC` — note: TouchOSC's sandboxed Lua has **no `pcall`**, so direct set-then-clear is the only option) plus a `receivingPatchTimer` frame-countdown backstop in `update()` that force-clears the flag a few frames later if `parseBlock` were ever to error mid-way. Suppresses the SysEx echo while still updating labels and BCR LED rings. |
...
| **CC16 (Accent)** | ✅ Wired (layout cleanup pending) | Turned out an on-screen control already existed — `accent_level_enc` in `vcf_group` (SysEx `10 00 14 0E`, id=264, "ACCENT LEVEL"). CC16 is just the hardwired standard-MIDI feedback the physical ACCENT knob sends alongside its SysEx patch write (same pattern as CC74/Cutoff and CC71/Resonance). Added `TB3_CC_DISPLAY_MAP[16] = "vcf_group,accent_level_enc"` in `root.lua` so the existing on-screen control now live-updates as the hardware knob turns — no new encoder group needed. **Layout cleanup**: a duplicate `accent_enc` group was added to `other_group` (alongside the relocated `pitch_bend_range_enc`) before this was discovered; remove it via the TouchOSC editor to avoid a redundant/unwired control (it has no Lua mapping and won't function). |
