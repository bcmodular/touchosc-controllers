#!/bin/bash
# Launch TB-3 preset manager utility.
# Uses system python3 when it has a working PyQt5 (common on macOS).
# The project .venv is optional (set TB3_PRESET_MANAGER_USE_VENV=1 to force it).
set -euo pipefail
cd "$(dirname "$0")"
ROOT="$(pwd)"
...
_pick_python() {
  if [[ "${TB3_PRESET_MANAGER_USE_VENV:-}" == "1" && -x "${ROOT}/.venv/bin/python" ]]; then
